...
const Scrollytelling = ({ videoSrc, chapters }) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Determine the active chapter by dividing the scroll into equal segments
      if (chapters && chapters.length > 0) {
        const segmentSize = 100 / chapters.length;
        const index = Math.max(0, Math.min(chapters.length - 1, Math.floor(scrollPercent / segmentSize)));
